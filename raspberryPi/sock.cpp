 ...
 * @param port 服务器端口号
 * @note 内部实现3次重试，失败时抛出异常（而非直接退出）
 */
void Socket::connectToServer(int *sock, char* ip, int port) {
    if (sock == nullptr || ip == nullptr || port < 1 || port > 65535) {
        throw std::invalid_argument("无效的参数：sock不可为空，IP不可为空，端口需在1-65535范围内");
    }
    ...
    while (retry_count < max_retries) {
        ...
        *sock = socket(AF_INET, SOCK_STREAM, 0);
        if (*sock < 0) {
            throw std::runtime_error("套接字创建失败：" + std::string(strerror(errno)));
        }
    ...
    // 超过最大重试次数仍失败
    throw std::runtime_error("超过最大重试次数（" + std::to_string(max_retries) + "次），连接服务器失败");
}
